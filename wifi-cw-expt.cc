...
#include "ns3/flow-monitor-helper.h"  //for contention window from example
...
#include "ns3/gnuplot.h"            // for plots from example
#include <cmath>
...
//changed from void to double because now we require to plot average throughput and that is to be returned from the experiment method.
double experiment (bool enableCtsRts, std::string wifiManager, double cwMin = 16, double cwMax= 64 )
{
  // 0. Enable or disable CTS/RTS
  // not required in contention window experiment, hence passing enableCtsRts to false
  ...
  lossModel->SetDefaultLoss (50); //moving forward with 50
  ...
  double avgTput = 0; // from cw_expt.cc
...
int main (int argc, char **argv)
{
  ...
  /* Commenting the below code for contention window experiment */
  ...
  // experiment (true, wifiManager);
    ...
    // from cw_Expt.cc
  std::ofstream plotFile ("cw_Expt_013533169.plt");
  GnuplotCollection gnuplots ("cw_Expt_013533169.pdf");
  Gnuplot plot;
  ...
  for(uint32_t cwExpMax = 6; cwExpMax <= 8; cwExpMax++){
    ...
    for(uint32_t cwExpMin = 4; cwExpMin <= 6; cwExpMin++){
       ...
       std::cout << "cwMin:"<< cwMin <<", cwMax:"<<cwMax<<"\n";   // printing the diferent conttention windows
    
        double avgTput = experiment(false, wifiManager, cwMin, cwMax); // returning from the experiment which we changed earlier
