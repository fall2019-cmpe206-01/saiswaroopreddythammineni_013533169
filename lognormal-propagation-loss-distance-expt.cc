...
int main (int argc, char *argv[])
{
  CommandLine cmd;
  cmd.Parse (argc, argv);
  std::ofstream plotFile ("output1.plt");
  
  //Set the random seed value
  RngSeedManager::SetSeed (3);  
  
  GnuplotCollection gnuplots ("rxPower_2.5_0_4.pdf");

  {
	...
	plot.AppendExtra ("set key outside");
      ...
      //Added for  Lognormalpropagation
      Ptr<LogNormalPropagationLossModel> logNormalPropagation = CreateObject<LogNormalPropagationLossModel> ();
      
      logNormalPropagation->SetAttribute("RandomVariable", StringValue ("ns3::NormalRandomVariable[Mean=0|Variance=4]"));
      
      logNormalPropagation->SetAttribute("Exponent",DoubleValue (3.0));
      
          for (double distance = 250.0; distance <= 400.0; distance += 50.0)
      {
          Gnuplot2dDataset dataset = TestProbabilistic (logNormalPropagation, distance);
      ...
      plot.SetTitle ("Log Normal Propagation Loss");
